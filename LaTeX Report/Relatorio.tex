...
Nesta secção, serão apresentados os fundamentos do SNMP, incluindo a sua definição, propósito, arquitetura e funcionamento básico. Esta base teórica é essencial para 
compreender as diferenças entre as versões do protocolo que serão analisadas posteriormente.
...
A arquitetura do SNMP baseia-se no modelo gestor-agente (\textit{manager-agent}), composto por quatro elementos principais:
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.9\textwidth]{Recursos/snmp_architecture.png}
	\caption{Arquitetura do SNMP}
...
O SNMP define um conjunto limitado de operações básicas, mantendo a simplicidade que caracteriza o protocolo. As principais 
operações são (\cite{rfc3416}):
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\textwidth]{Recursos/snmp_operations.png}
	\caption{Operações do SNMP}
	\label{fig:nfswarning}
\end{figure}
...
Nesta secção, serão analisadas as três principais versões do SNMP: SNMPv1, SNMPv2 e SNMPv3. Para cada versão, serão discutidas as suas características técnicas, vantagens e desvantagens, com ênfase especial na evolução dos mecanismos de segurança ao longo do tempo.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\textwidth]{Recursos/v1_message.png}
	\caption{Formato do SNMPv1}
	\label{fig:nfswarning}
\end{figure}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\textwidth]{Recursos/v2_message.png}
	\caption{Formato do SNMPv2}
	\label{fig:nfswarning}
\end{figure}
...
O SNMPv3, definido nas RFC 3410-3415, foi publicado em 1998 e representa a evolução mais significativa do protocolo, focando-se primariamente em segurança (\cite{rfc3411}). Esta versão não substitui o SNMPv2, mas adiciona capacidades de segurança robustas.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\textwidth]{Recursos/v3_message.png}
	\caption{Formato do SNMPv3}
	\label{fig:nfswarning}
\end{figure}
...
\begin{tabular}{|p{4cm}|p{2.5cm}|p{2.5cm}|p{3.0cm}|}
\hline
\textbf{Característica} & \textbf{SNMPv1} & \textbf{SNMPv2c} & \textbf{SNMPv3} \\
\hline
Ano de publicação & 1988 & 1993/1996 & 1998 \\
\hline
Autenticação & Comunidade & Comunidade & USM com HMAC \\
\hline
...
Em suma, o SNMP estabeleceu-se como padrão de facto para gestão de redes IP, mantendo-se relevante por mais de três décadas. A sua evolução em três versões principais mostra um equilíbrio entre funcionalidade, segurança e simplicidade: o SNMPv1 definiu os fundamentos, simples e eficiente, porém com segurança limitada; o SNMPv2c trouxe melhorias como GET-BULK e contadores de 64 bits, mantendo desempenho mas ainda com proteção insuficiente; e o SNMPv3 introduziu segurança robusta, adequado para ambientes críticos, embora com maior complexidade e overhead. 

A escolha da versão deve considerar segurança, compatibilidade, desempenho e recursos disponíveis, sendo recomendado o SNMPv3 sempre que possível, aplicando medidas compensatórias quando versões antigas forem inevitáveis. 
\newpage
Apesar de suas limitações iniciais, o SNMP continua amplamente utilizado e demonstra como um protocolo pode evoluir para atender às necessidades modernas de redes, mantendo os princípios de simplicidade que motivaram sua criação.
